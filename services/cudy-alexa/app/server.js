...
async function setGuest(band,enabled){const b=String(band||'todas').toLowerCase().replace(/\s/g,'');let ids;if(/todas|ambas/.test(b))ids=[E.guest24,E.guest5];else if(['2.4','2,4','2.4ghz','24'].includes(b))ids=[E.guest24];else if(['5','5ghz'].includes(b))ids=[E.guest5];else throw new Error('Faixa de convidados inválida');for(const id of ids){const before=await state(id);if(!available(before))throw new Error(`${id} indisponível`);if(isOn(before)!==enabled)await service('switch',enabled?'turn_on':'turn_off',id);const after=await state(id);if(!available(after)||isOn(after)!==enabled)throw new Error(`HA não confirmou ${id}`);}}
...
const launchHandler={canHandle:h=>Alexa.getRequestType(h.requestEnvelope)==='LaunchRequest',handle:()=>response('Posso informar o status da rede, controlar a rede de convidados e reiniciar o roteador.')};
const networkStatus={canHandle:h=>Alexa.getRequestType(h.requestEnvelope)==='IntentRequest'&&Alexa.getIntentName(h.requestEnvelope)==='NetworkStatusIntent',async handle(){try{const[c,w,v]=await Promise.all([state(E.clients),state(E.wisp),state(E.vpn)]);return response(`Há ${available(c)?c.state:'um número desconhecido de'} dispositivos conectados. O WISP está ${spoken(w)} e a VPN está ${spoken(v)}.`);}catch(e){console.error('network status:',e.message);return response('Não consegui consultar o Home Assistant agora.');}}};
const detailStatus={canHandle:h=>Alexa.getRequestType(h.requestEnvelope)==='IntentRequest'&&Alexa.getIntentName(h.requestEnvelope)==='NetworkDetailIntent',async handle(h){const cat=slot(h,'Category').toLowerCase();const map={sistema:[E.uptime,'tempo de atividade'],lan:[E.lan,'IP da LAN'],dispositivos:[E.clients,'dispositivos conectados'],wifi:[E.channel,'canal Wi-Fi'],convidados:[E.guest24,'rede de convidados 2.4 GHz'],wisp:[E.wisp,'WISP'],vpn:[E.vpn,'VPN']};const[id,label]=map[cat]||[E.uptime,'tempo de atividade'];try{return response(`${label}: ${spoken(await state(id))}.`);}catch(e){return response(`Não consegui consultar ${label} agora.`);}}};
const guestControl={canHandle:h=>Alexa.getRequestType(h.requestEnvelope)==='IntentRequest'&&Alexa.getIntentName(h.requestEnvelope)==='GuestWifiControlIntent',handle(h){const action=slot(h,'Action').toLowerCase(),band=slot(h,'Band')||'todas';if(!permitted('control','guest_wifi'))return response('O controle da rede de convidados está desabilitado.');if(!['ligar','desligar'].includes(action))return response('Diga ligar ou desligar a rede de convidados.');return confirmation(`Você pediu para ${action} a rede de convidados em ${band}. Diga confirmar para executar ou cancelar.`,{type:'guest',action,band});}};
...
const helpHandler={canHandle:h=>Alexa.getRequestType(h.requestEnvelope)==='IntentRequest'&&Alexa.getIntentName(h.requestEnvelope)==='AMAZON.HelpIntent',handle:()=>response('Você pode perguntar o status da rede, WISP ou VPN, controlar a rede de convidados ou reiniciar o roteador.')};
const fallbackHandler={canHandle:()=>true,handle:()=>response('Não reconheci esse comando.')};
