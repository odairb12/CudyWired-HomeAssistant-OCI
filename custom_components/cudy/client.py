...
def _input(doc: str, name: str) -> str | None:
    return _form_fields(doc).get(name)

def _form_fields(doc: str) -> dict[str, str]:
    """Extract successful LuCI form controls without dropping select/textarea values."""
    out: dict[str, str] = {}
    for tag in re.findall(r'<input\b[^>]*>', doc, re.I):
        ...
        if typ in ('checkbox', 'radio') and not re.search(r'\bchecked(?:\s*=|\s|>)', tag, re.I):
            continue
        v = re.search(r'value=["\']([^"\']*)', tag, re.I)
        out[html_lib.unescape(n.group(1))] = html_lib.unescape(v.group(1)) if v else ('on' if typ in ('checkbox','radio') else '')
    for match in re.finditer(r'<select\b([^>]*)>([\s\S]*?)</select>', doc, re.I):
        n = re.search(r'name=["\']([^"\']+)', match.group(1), re.I)
        if not n:
            continue
        selected = re.search(r'<option\b([^>]*\bselected\b[^>]*)>([\s\S]*?)</option>', match.group(2), re.I)
        if not selected:
            selected = re.search(r'<option\b([^>]*)>([\s\S]*?)</option>', match.group(2), re.I)
        if selected:
            v = re.search(r'value=["\']([^"\']*)', selected.group(1), re.I)
            out[html_lib.unescape(n.group(1))] = html_lib.unescape(v.group(1) if v else re.sub(r'<[^>]+>', '', selected.group(2)).strip())
    for match in re.finditer(r'<textarea\b([^>]*)>([\s\S]*?)</textarea>', doc, re.I):
        n = re.search(r'name=["\']([^"\']+)', match.group(1), re.I)
        if n:
...
class CudyClient:
    ...
    def __init__(self, host: str, username: str, password: str, verify_ssl: bool = True):
        ...
        if parsed.scheme not in ('http', 'https') or not parsed.hostname:
            raise ValueError('Invalid Cudy URL')
    ...
    async def async_open(self):
        ...
        self.session = aiohttp.ClientSession(timeout=timeout, cookie_jar=aiohttp.CookieJar(unsafe=True), connector=aiohttp.TCPConnector(ssl=self.verify_ssl), headers={'User-Agent':'HomeAssistant-Cudy/0.2'})
    ...
    async def _request(self, method: str, path: str, *, data=None, retry_get=True):
        ...
        try:
            async with self.session.request(method, self.base + path, data=data, allow_redirects=False) as r:
                ...
                if r.status in (301,302,401,403) or re.search(r'name=["\']luci_password', body, re.I): raise CudySessionExpired()
            ...
            if method == 'GET' and retry_get:
                await asyncio.sleep(random.uniform(.15,.6)); return await self._request(method,path,data=data,retry_get=False)
    ...
    async def async_login(self, force=False):
        async with self._auth_lock:
            ...
            try:
                async with self.session.get(self.base+'/cgi-bin/luci/',allow_redirects=False) as r: doc=await r.text(errors='replace')
                ...
                async with self.session.post(self.base+'/cgi-bin/luci/',data=form,allow_redirects=False) as r: body=await r.text(errors='replace')
            ...
            except (aiohttp.ClientError, asyncio.TimeoutError) as exc: raise CudyCannotConnect(str(exc)) from exc
    ...
    async def async_get(self,path:str):
        ...
        try: return await self._request('GET',path)
            ...
            await self.async_login(force=True); return await self._request('GET',path)
    ...
    def _parse_clients(self,doc:str):
        ...
        for block in blocks:
            text=_text(block)
            mac_match=re.search(r'(?i)\b(?:[0-9a-f]{2}:){5}[0-9a-f]{2}\b',text)
            ...
            before=text[:mac_match.start()].strip(' -|:')
            hostname=(before.split()[-1] if before else None) or ip or f'device-{len(devices)+1}'
            devices.append(CudyClientDevice(key=mac,mac=mac,ip=ip,hostname=hostname))
        return devices

    def _assert_write_supported(self):
